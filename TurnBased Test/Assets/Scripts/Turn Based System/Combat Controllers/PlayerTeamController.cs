...
public class PlayerTeamController : MonoBehaviour
{
    ...
    List<RealtimeCombatant> _realtimeCombatantTeam = new List<RealtimeCombatant>();
    List<CombatantInfoUI> _allCharacterInfoUI = new List<CombatantInfoUI>();

    [SerializeField] List<RecruitedCharacter> _savedPlayerTeam = new List<RecruitedCharacter>();

    void Awake()
    {
        _matchManager.TurnStateChange += MatchTurnStateChanged;
        _matchManager.AllCombatantsCreated += TeamCreated;
    ...
    void SetupTeamSkills()
    {
        for (int i = 0; i < _savedPlayerTeam.Count; i++)
        {
            List<SkillInfo> skills = new List<SkillInfo>();

            foreach (var equipment in _savedPlayerTeam[i].heldEquipment)
                skills.Add(equipment.awardedSkill);

            _realtimeCombatantTeam[i].SetupCombatantSkills(skills);
        }
    }
    ...
    {
        List<CharacterInfo> teamInfo = new List<CharacterInfo>();

        foreach (var playerTeamMember in _savedPlayerTeam)
            teamInfo.Add(playerTeamMember.characterInfo);
